...
data = np.array(pd.read_csv('protein.csv', header = None))
labels = np.array(pd.read_csv('protein_labels.csv', header = None))

metric_2d = manifold.MDS(n_components = 2, metric = True)
nonmetric_2d = manifold.MDS(n_components = 2, metric = False)
metric_131d = manifold.MDS(n_components = 131, metric = True)
nonmetric_131d = manifold.MDS(n_components = 131, metric = False)
...
# recon_data_nm131d = metrics.pairwise_distances(nm131d)
# np.savetxt('nmMDS_131d.csv', recon_data_nm131d, delimiter=',')
